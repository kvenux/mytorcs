...
def playGame(env_num, lock, epoch):    #1 means Train, 0 means simply Run
    BUFFER_SIZE = 100000
    BATCH_SIZE = 32
    GAMMA = 0.99
    ...
    env = TorcsEnv(env_num, lock, vision=True, throttle=True, gear_change=False)
    # time.sleep(4 * (12 - env_num))
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            # print("%.2f"%a_t[0][0])
            # a_t[0][2] += 0.7
            recording_obs.append(ob.img)
            recording_action.append(a_t[0])
        ...
        recording_obs = np.array(recording_obs[:1000], dtype=np.uint8)
        recording_action = np.array(recording_action[:1000], dtype=np.float16)
        ...
        # with open('logs/{}_{}_{}.csv'.format(model_name, env_num, epoch), 'a') as the_file:
        #     the_file.write(s)
...
if __name__ == "__main__":
    ...
    for epoch in range(100):
        for i in range(thread_num):
            pro = Process(target = playGame, args=(i, lock, epoch, ))
            pro.start()
