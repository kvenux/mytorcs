...
def playGame(train_indicator=0):    #1 means Train, 0 means simply Run
    ...
    actor_pro = ActorNetwork(sess, state_dim, action_dim, BATCH_SIZE, TAU, LRA)
    critic_pro = CriticNetwork(sess, state_dim, action_dim, BATCH_SIZE, TAU, LRC)

    #Now load the weight
    load_name = "sample_v0_40"
    print("Now we load the weight")
    actor_pro.model.load_weights("saved/actormodel_{}.h5".format(load_name))
    critic_pro.model.load_weights("saved/criticmodel_{}.h5".format(load_name))
    actor_pro.target_model.load_weights("saved/actormodel_{}.h5".format(load_name))
    critic_pro.target_model.load_weights("saved/criticmodel_{}.h5".format(load_name))
    print("Weight load successfully")

    plt.figure()
    overall_scores = []
    model_name = "auto_adv_v1"
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            a_t_pro = actor_pro.model.predict(s_t.reshape(1, s_t.shape[0]))

            if a_t[0][2] > 0.75 and attack_valid > 0 and j > 30:
                a_t_pro[0][0] = a_t[0][0]
                a_t_pro[0][1] = a_t[0][1]
                print("adv!")
                attack_valid -= 1

            ob, r_t, done, info = env.step(a_t_pro[0])
            print "step: {} reward: {:.5f} action: {:.5f} {:.5f} {:.5f} ".format(j, r_t, a_t[0][0], a_t[0][1], a_t[0][2])
            # print "{:.5f} {:.5f} {:.5f} ".format(ob.angle, ob.trackPos, a_t[0][0])
