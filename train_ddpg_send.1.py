...
# socket_bind.connect((TCP_IP, TCP_PORT))
...
def playGame(train_indicator=0):    #1 means Train, 0 means simply Run
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            target_speed = 0.3
            speed_p = 1.0
            a_t[0][1] = (target_speed - ob.speedX)*speed_p
            
            angle_theta = 0.1
            dis_theta = 0.1
            # target_steer = angle_theta*abs(ob.angle) + dis_theta*abs(ob.trackPos)
            # if a_t[0][1] > 0.5:
            #     a_t[0][1] -= 0.3
            # if ob.trackPos > 0.6:
            #     target_steer = 0
            #     if ob.angle < 0.1:
            #         target_steer += (0.1 - ob.angle)*angle_theta
            #     a_t[0][0] = - target_steer
            #     a_t[0][1] = 0.3
            #     a_t[0][2] = 0.1
            #     if ob.speedX < 0.1:
            #         a_t[0][2] = 0.0
            ...
            #     target_steer = 0
            #     if ob.angle > -0.1:
            #         target_steer += (ob.angle + 0.1)*angle_theta
            #     a_t[0][0] = target_steer
            #     a_t[0][1] = 0.3
            #     a_t[0][2] = 0.1
            #     if ob.speedX < 0.1:
            #         a_t[0][2] = 0.0
            target_pos_index = np.argmax(ob.track)
            max_dis_list = []
            for i in range(len(ob.track)):
                if ob.track[i] == 1.0:
                    max_dis_list.append(i)
            if len(max_dis_list) > 0:
                target_pos_index = np.mean(np.array(max_dis_list))

            target_angle = (target_pos_index - 9)/9.0*(-np.pi/2)
            a_t[0][0] = angle_theta*(target_angle - ob.angle)

            ob, r_t, done, info = env.step(a_t[0])
            # track_arr = ["%.3f"%val for val in ob.track]
            # track_arr.append("%.3f"%(ob.angle))
            # message = ','.join(track_arr)
            # socket_bind.send(message)
            # print "step: {} reward: {:.5f} action: {:.5f} {:.5f} {:.5f} ".format(j, r_t, a_t[0][0], a_t[0][1], a_t[0][2])
            print "{:.5f} {:.5f} {:.5f} ".format(ob.angle, target_angle, ob.speedX)

            # print "{:.5f} {:.5f} {:.5f} {:.5f} {:.5f}".format(r_t, ob.speedX, ob.speedY, ob.speedZ, ob.rpm)
            # if(r_t < -50):
            #     r_t -= 10000
            #     done = True
            if j > 20 and ob.speedX <= 0.001:
                r_t -= 1000
                done = True
