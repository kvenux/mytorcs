...
def playGame(train_indicator=0):    #1 means Train, 0 means simply Run
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            # if j > 120:
            flag = 1 if np.random.random() < epsilon and j >= 30 else 0
            noise_t[0][0] = train_indicator * flag * OU.function(a_t_original[0][0],  0.0 , 1.0, 0.30)
            noise_t[0][1] = 0
            noise_t[0][2] = 0
            ...
            if (train_indicator):
                ...
                for a_index in range(len(next_actions)):
                    ...
                    cur_next_state_array = np.array([dangerQ_new_states[a_index] for k in range(21)])
                    cur_target_q_pre = dangerQ_critic.target_model.predict([cur_next_state_array, next_action_array])
                ...
                for k in range(len(dangerQ_batch)):
                    if dones[k]:
                        ...
                        dangerQ_y_t[k] = dangerQ_rewards[k] + GAMMA*dangerQ_target_q_values[k]
                dangerQ_critic.model.train_on_batch([dangerQ_states, dangerQ_actions], dangerQ_y_t)
        ...
        if np.mod(i, 3) == 0:
            if (train_indicator):
                ...
                dangerQ_critic.model.save_weights("saved/dangerQ_criticmodel_{}_{}.h5".format(model_name, int(step/10000)), overwrite=True)
